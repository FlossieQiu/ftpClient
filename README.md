...
在使用前，请确保插件描述文件（`PluginFtpClient.txt`）及对应的二进制文件（`.dll` 或 `.so`）已放置在服务器插件目录中。
...
- data: 字符串、Blob 或 CHAR Vector。需要上传的数据内容。
- user: 字符串（可选）。用户名，默认为空。
- password: 字符串（可选）。密码，默认为空。
- timeout: 数值（可选）。超时时间（秒），默认 30.0。
#### 返回值
...
```
## 2.3 ftpClient::ftpUploadFile
将本地磁盘文件上传到 FTP 服务器。采用流式传输，内存占用极低，适合上传大文件。
#### 语法
ftpClient::ftpUploadFile(url, localFilePath, [user], [password], [timeout])
参数
- url: 字符串。完整的目标路径，必须包含文件名。
- localFilePath: 字符串。本地文件的绝对路径。支持 Windows 路径（如 D:/data/file.zip）。
...
### 2.4 ftpClient::ftpDownloadDir
功能强大的通用下载函数。支持下载单个文件或递归下载整个目录，并内置断点续传机制。
#### 语法
ftpClient::ftpDownloadDir(remoteUrl, localDir, [user], [password], [timeout])
#### 参数
- remoteUrl: 字符串。
...
- timeout: 数值（可选）。超时时间（秒），默认 30.0。
#### 返回值
INT。成功下载或校验通过（跳过）的文件总数。
#### 核心特性
 - 自动探测：插件会自动判断 remoteUrl 是文件还是目录，无需手动指定。
 - 断点续传与增量同步：
   - 跳过：如果本地文件大小 == 远程文件大小，视为已完成，直接跳过（节省带宽）。
   - 续传：如果本地文件大小 < 远程文件大小，从断点处继续下载（追加模式）。
   - 覆盖：如果本地文件大小 > 远程文件大小，视为异常，覆盖重新下载。
- 智能编码处理：
...
## 3. 常见问题 (FAQ)
### Q: Windows 下下载中文文件名出现乱码或无法创建文件？
**A**: 本插件内置了编码转换逻辑。在从 URL (UTF-8) 保存到 Windows 文件系统时，会自动转换为 ANSI (GBK) 编码。请确保 FTP 服务器发送的是标准的 UTF-8 文件名（现代 FTP 服务器如 pyftpdlib, vsftpd 默认均为 UTF-8）。如果服务器使用非标编码（如 GBK），可能仍会出现乱码。

### Q: 遇到 Access denied 或 550 错误？
**A**: 请检查：
- FTP 服务器用户是否有对应的写入（上传时）或读取（下载时）权限。
- URL 路径拼写是否正确。
- 如果是上传，确保 FTP 服务器允许上传。
### Q: 为什么 ftpList 有时不显示精确的秒级时间？
...
### Q: 我传入的 URL 包含空格或中文，需要自己手动进行 URL Encode 吗？
